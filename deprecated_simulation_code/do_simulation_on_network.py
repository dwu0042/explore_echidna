...
def run_temporal_sim(
    G, sizes, prob_final_stay, parameters, num_seed_events=3, num_seed_indvs=5
):
...
def do_temporal_sim(
    graph_file, size_file=None, prob_final_file=None, parameters=(0.2, 1.0 / 28)
):
...
def run_snapshot_sim(
    sizes, snapshots, prob_final, parameters, num_seed_events=3, num_seed_indvs=5
):
...
def do_snapshot_sim(
    ...
    size_file=None,
    graph_file=None,
    prob_final_file=None,
    parameters=(0.2, 1.0 / 28),
):
...
def run_batched_temporal_simulations(
    graph_file,
    size_file=None,
    prob_final_file=None,
    parameters=(0.2, 1.0 / 365, 1.0 / 28),
    seed_options=(3, 5),
    n_runs=10,
):
    ...
    return [
        run_temporal_sim(graph, sizes, prob_final, parameters, *seed_options)
        for _ in range(n_runs)
    ]


def run_batched_snapshot_simulations(
    snapshot_file,
    size_file=None,
    graph_file=None,
    prob_final_file=None,
    ...
    snapshots = ons.load_snapshots(snapshot_file)
    assert not (
        size_file is None and graph_file is None
    ), "Need to provide either the hospital size file, or a temporal graph file."
    sizes = find_sizes(size_file, graph_file)
    prob_final = load_prob_final_file(prob_final_file)

    return [
        run_snapshot_sim(sizes, snapshots, prob_final, parameters, *seed_options)
        for _ in range(n_runs)
    ]


def run_static_sim(
    graph, sizes, prob_final, parameters, n_seed_events, n_seeds_per_event
):
    ...
    sim = ons.Simulation(
        ordered_sizes, transition_matrix, prob_final_arr, parameters=parameters, dt=1
    )
...
def run_static_simulations(
    ...
    return [
        run_static_sim(G, sizes, prob_final, parameters, *seed_options)
        for _ in range(n_runs)
    ]
...
class SimulationResult:
    ...
    def minify(cls, fullsim: ons.Simulation):
        return cls({"ts": fullsim.ts, "history": fullsim.history})


def export_batched_realisations(
    realisations: Sequence[ons.Simulation | SimulationResult], metadata, output_path
):
...
if __name__ == "__main__":
    main(
        ...
        "./concordant_networks/size_14.csv",
        "probability_of_final_stay_by_shuffled_campus.csv",
        parameters=(0.2,),
    )
