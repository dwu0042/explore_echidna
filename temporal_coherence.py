...
def extract_all_nodes(path: str):
    ...
    return {k:i for i, k in enumerate(sorted({int(x.split(',')[0].lstrip('(')) for x in TN.vs['name']}))}
...
def permutation_matrix(G: ig.Graph, all_nodes: Mapping):
    ...
    P = sparse.COO([this_index, that_index], 
...
def burstiness_distribution(C: sparse.COO):
    W = waiting_times(C)
    distribution = [burstiness(v) for (k0,k1),v in W.items() if len(v) > 1 if k0 != k1]
    return distribution
