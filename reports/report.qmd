...
This implies that the rate movement of patients between hospitals is Poisson, or that the time between successive admissions for a given patient is exponentially distributed. Empirically, this is poor modelling choice, since the readmission window, as shown in @fig-readmission-window, is definitely not exponentially distributed, and has a significant heavy tail. A naïve static approximation that matches the mean readmission window will be inaccurate; in the following, we will attempt to resolve this problem in three increasingly granular ways:
...
Patient transfers can be considered as edges from some source hospital $x_i$ at some time $t_i$ to a target hospital $x_j$ at some time $t_j$. We could work with the exact times, but this will lead to an overly sparse representation that is difficult to work with.
Instead, we discretise time into bins by treating movements involving some arbitrary hospital $x_i$ between times $t_i$ and $t_i + W$ as instead involving a single node at $v_i \coloneqq (x_i, t_i)$. 
