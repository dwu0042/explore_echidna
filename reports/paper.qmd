...
stay_dur_pow_alpha = estimate_alpha(*stay_dur_xfx, xmin=40)
...
$x_\text{min}$ is estimated by a grid search, and looking the region where the corresponding $\hat{\alpha}\text{s}$ are stable.
...
axz[1].loglog(xmin_arr, alpha_hats)
axz[1].set_xlabel(r'$x_\text{min}$')
axz[1].set_ylabel(r'$\hat\alpha$')

axz[0].scatter(*stay_dur_xfx, alpha=0.3, color='k', zorder=999)
...
xgrid_norm = mcolors.Normalize(vmin=min(xmin_arr), vmax=max(xmin_arr))
...
fig.colorbar(xgrid_clrs, ax=axz[0])
```

For readmission, preliminary linear regression on log-log shows that we have $\alpha < 1$, so we cannot use the result from @clauset_power-law_2009. 
```{python}
...
axs['datafit'].scatter(*readmit_xfx, alpha=0.3, color='k', zorder=997)
...
r_xmin_norm = mcolors.Normalize(vmin=min(r_xmin_arr), vmax=max(r_xmin_arr))
...
fig.colorbar(r_xmin_clrs, ax=axs['datafit'])
```
