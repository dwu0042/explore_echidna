...
One of these is the addition of disease transmission dynamics. We chose to neglect these mechanisms due to the complexity of defining and characterising the manner of transmission within a facility -- and this can have confounding effects on the interpretation of observed results. It is believed that CPE, for example, is transmitted through the environment, with 

In the current study, we have used a discrete time stepping scheme in order to allow for some non-Markovian behaviour; it is likely that an agent-based model would be a more "accurate" representation. This would allow for "higher-order" behaviours, such as those observed in [@myall_network_2021], where patient pathways through wards in that case, and hospitals in ours, are dependent on their admission history.
However, agent-based models are an order of magnitude more expensive to simulate, due to increased computational and memory requirements. The number of 
