...
The temporal characterisation of the Victorian healthcare system is less well-explored.
For example, the amount of patient movement in the system fluctuates temporally, as shown in @fig-movement-over-time. 
...
The underlying admission duration is ___ distributed (see @fig-stay-duration), and the time between admissions (i.e. the transfer period) is power-law distributed (see @fig-readmission-time). Thus, a fully Markovian POisson process on a static network is unlikely to be a good representation of the movement in the network.
