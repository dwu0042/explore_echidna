...
To motivate the following methods, we first characterise temporal trends in the data.

Firstly, we examine the duration of stays 


```{python}
#| label: fig-stay-duration
#| fig-cap: Distribution of duration of stays

stay_dur_df = pl.read_csv(root / "data/export/stay_duration_hist.csv")

fig, ax = plt.subplots()
sns.scatterplot(stay_dur_df, x='duration', y='count')
```

In this paper, we investigate these differences using the Victorian hospital system as a baseline for comparison. Qualitatively, there are mechanisms implicit in the data that are not previously well-modelled, and may be of importance to constructing robust control measures. Critically, we notice that the distribution of readmission times (the time from a discharge of a given patient from a facility to next admission of that patient at any facility), as plotted in @fig-readmission-time, are not well modelled by an exponential distribution, i.e. poorly approximated by Markov processes.
