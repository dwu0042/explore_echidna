...
Admissions data can be used to inform the construction of a wide variety of (weighted, directed) networks, which can, in turn, be used to construct a stochastic process model. In this study, we use discrete time Markov chain models, where a state vector $x$ that represents the number of marked individuals at facilities $u$. Weights $w_{ij}$ of the edges $e_{ij}$ between nodes $u_i, u_j$ of the network inform the hazard function of the movement process of marked individuals between those nodes.
...
The governing equations for such models can be captured by sets of transitions modelled as a Markov chain. The hazards of the transition events is given in @eq-general-model-transitions.
...
In order in increase the flexibility of the models, we will use a _discrete time_ Markov chain, where we have a Poisson-distributed number of transition events occur within some given timestep. For some models, we will also allow for the number of transitions for a particular class of event to be non-Poisson.

<!--# Model abstraction ends here, model details start here -->
...
In the current study, we have used a discrete time stepping scheme in order to allow for some non-Markovian behaviour; it is likely that an agent-based model would be a more "accurate" representation. 
We have also neglected "higher-order" pathing. [@myall_network_2021]
